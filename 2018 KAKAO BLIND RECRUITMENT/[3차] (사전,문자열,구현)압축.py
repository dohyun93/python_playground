...
def solution(msg):
    ...
    for idx in range(1, 27):
        myDict[idx] = chr(ord('A')+idx-1)
    # chr(정수) -> ASCII값으로 변환.
    # ord(문자) -> 문자의 ASCII정수값으로 변환.
    lastIdx = 27
    curIdx = 0
    while curIdx < len(msg):
        # 1. curIdx부터 가장 긴 존재하는 길이의 문자열 찾기.
        # 2. 해당 문자열에 대한 색인 answer에 append.
        # 3. 해당 문자열 + 1자리까지 새로 등록.
        # 4. +1자리 부터 다시 1단계 반복.
        for right in range(curIdx, len(msg)):
            curString = msg[curIdx:right+1]
            thisKey = getKey(myDict, curString)
            # 이 문자열은 없음.
            if thisKey == None:
                answer.append(getKey(myDict, curString[:-1]))
                myDict[lastIdx] = curString
                curIdx = right
                lastIdx += 1
                break
